...
export default function AdminOrders() {
  ...
  const code = (o.orderCode || generateOrderCode(o.id)).toLowerCase();
 ...
 const getStatusColor = (status: Order['status']) => {
  switch (status) {
    ...
    case 'CancelRequested': return 'bg-orange-100 text-orange-700 border-orange-200';
    case 'CancelRejected': return 'bg-rose-100 text-rose-700 border-rose-200';
  ...
  const orderCode = order.orderCode || generateOrderCode(order.id);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
           ...
           <p className="text-sm text-slate-400">Pantau semua pesanan masuk dari WhatsApp katalog.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          [1,2,3].map(i => <div key={i} className="h-64 bg-slate-100 animate-pulse rounded-xl" />)
        ) : filteredOrders.length > 0 ? (
          filteredOrders.map((order) => (
            <div key={order.id} className="bg-white rounded-xl shadow-soft border border-slate-100 overflow-hidden flex flex-col group">
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          navigator.clipboard.writeText(order.orderCode || generateOrderCode(order.id));
               ...
               <div className="p-6 flex-grow space-y-5">
                  <div>
                    <h4 className="text-sm font-black text-slate-800 mb-1">{order.productName}</h4>
                    <p className="text-brand-pink-dark font-bold text-sm tracking-tight">{formatPrice(order.productPrice)}</p>
                  </div>
                  ...
                  <div className="space-y-3 pt-4 border-t border-slate-50">
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <a href={`https://wa.me/${order.customerWhatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener" className="text-xs font-bold text-blue-500 hover:underline flex items-center gap-1">
                  ...
                  {order.note && (
                    <div className="p-3 bg-brand-cream/50 rounded-lg border border-brand-pink-dark/5">
                       <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Catatan:</p>
          ...
          <div className="col-span-full py-20 text-center bg-white rounded-xl border-2 border-dashed border-slate-100">
             ...
             <p className="text-slate-400 text-sm font-medium italic">Belum ada pesanan yang sesuai.</p>
          </div>
        )}
      </div>
    </div>
